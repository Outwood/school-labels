...
class AttendanceTemplate(Avery7160Template):
    ...
    COLOUR_MAP: ClassVar[dict[str, tuple[DeviceCMYK, DeviceCMYK]]] = {
    ...
    TREND_MAP: ClassVar[dict[str, tuple[str, DeviceCMYK]]] = {
    ...
    def _draw_trend(
        ...
        """Draw trend text if the trend key is recognised."""
        if trend_key not in self.TREND_MAP:
            return
    ...
    def _draw_colour_banner(
        self, pdf: FPDF, x: float, banner_y: float, height: float, colour_key: str
    ) -> None:
        """Draw the colour banner at the bottom of the label."""
        if colour_key not in self.COLOUR_MAP:
            return
        bg_colour, txt_colour = self.COLOUR_MAP[colour_key]
        pdf.set_fill_color(bg_colour)
        pdf.rect(x, banner_y, self.LABEL_WIDTH, height, "F")
        pdf.set_font("Helvetica", "B", 12)
        pdf.set_text_color(txt_colour)
        pdf.set_xy(x, banner_y)
        banner_text = f"{colour_key.title()} attendance"
        pdf.cell(self.LABEL_WIDTH, height, banner_text, align="C")
        pdf.set_text_color(_BLACK)
    ...
    def _draw_label_content(
        ...
        try:
            attendance_display = f"{float(attendance_raw.rstrip('%')):.2f}%"
        except ValueError:
            attendance_display = ""
        half_width = full_width / 2
        pdf.cell(half_width, 4.2, attendance_display)

        trend_key = str(data.get("trend", "")).lower()
        self._draw_trend(pdf, trend_key, content_x, current_y, half_width)

        # Colour banner — full label width, bottom of label
        current_y += 6.0
        banner_height = (y + self.LABEL_HEIGHT) - current_y - 1.75  # 5pt bottom margin
        colour_key = str(data.get("colour", "")).lower()
        self._draw_colour_banner(pdf, x, current_y, banner_height, colour_key)
